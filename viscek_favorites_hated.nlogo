...
to go
  ...
  ask turtles [
    ...
    let favs-in-r turtles in-radius special-radius with [favorite? = true]
    if ( count favs-in-r ) > 0 [
      ask favs-in-r [
        set theta 0.2 * theta + 0.8 * heading
      ]
    ]
    if member? [who] of hated [who] of turtles in-radius special-radius [
      set theta 0.2 * theta - 0.8 * [heading] of hated
    ]
    ifelse self = hated [
      set heading 0 - theta
    ] [
  ...
  set std standard-deviation [heading] of turtles
  ...
  ask turtles [
    if cluster-number = 0 [
      set max-num max-num + 1
      set cluster-number max-num
      rec-find-clust self
    ]
  ]
...
to rec-find-clust [turt]
  ask other turtles in-radius cluster-radius [
    if cluster-number != max-num [
      set cluster-number max-num
      rec-find-clust self
    ]
  ]
...
"" ""
PENS
"std" 1.0 0 -16777216 true "" "plot std"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot max-num"
