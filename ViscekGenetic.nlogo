...
doves-own [
  ...
  n-children
  flock-assigned
  genetic-friendly-radius
  genetic-heading-noise-range
  genetic-dove-standard-speed
  genetic-dove-enemy-vision
  genetic-dove-boost-speed
  genetic-dove-fear-dir-noise
  genetic-threat-ignorance
]
...
to breed-doves
  ask doves [
    if n-children < max-children[
      let spouse min-one-of other doves in-radius genetic-friendly-radius with [n-children < max-children] [distance myself]
      if spouse != nobody [
        give-birth spouse
...
to update-doves
  ...
  ask doves [
    ...
    let theta heading
    let a doves in-radius genetic-friendly-radius
    if a != nobody [
      set theta average-flockmate-heading a
    ]
    let in-flock-heading add-noise-to-heading genetic-heading-noise-range theta

    if-else any? hawks in-radius genetic-dove-enemy-vision [
      ;take into accoutn the presence of hawks and adjust the direction
      let threats hawks in-radius genetic-dove-enemy-vision
      let avg-hawk-direction optimal-hawk-avoiding-heading threats
      let fear-heading add-noise-to-heading genetic-dove-fear-dir-noise avg-hawk-direction

      let optimal-heading (((in-flock-heading - 180) * genetic-threat-ignorance + (fear-heading - 180) * (1 - genetic-threat-ignorance)) + 180)
      set heading optimal-heading

      fd genetic-dove-boost-speed + genetic-dove-standard-speed
    ][
      ;stay with the flock
      set heading in-flock-heading

      fd genetic-dove-standard-speed
    ]
 ]
...
to define-clusters
  ...
  set c (count doves with [my-area < max-initial-friendly-radius * max-initial-friendly-radius * pi]) / (count doves)
end
